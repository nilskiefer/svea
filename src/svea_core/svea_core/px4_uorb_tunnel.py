...
class UorbSchemaRegistry:
    ...
    def _load_raw_types(self) -> None:
        if not self._msg_dir.exists() or not self._msg_dir.is_dir():
            raise RuntimeError(
                f"px4_msg_dir does not exist or is not a directory: {self._msg_dir}"
            )

        msg_files = sorted(self._msg_dir.glob("*.msg"))
        if not msg_files:
            msg_files = sorted(self._msg_dir.rglob("*.msg"))
        if not msg_files:
            raise RuntimeError(f"No .msg files found in px4_msg_dir: {self._msg_dir}")

        for msg_path in msg_files:
            type_name = msg_path.stem
            if type_name in self._raw_types:
                raise RuntimeError(
                    f"Duplicate message type '{type_name}' in {msg_path}"
                )
...
class Px4UorbTunnelNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self.declare_parameter("mavlink_topic", "/uas1/mavlink_source")
        ...
        if not self._px4_msg_dir:
            self._px4_msg_dir = self._discover_px4_msg_dir()
        if not self._px4_msg_dir:
            raise RuntimeError(
        ...
        self.create_subscription(
            Mavlink, self._mavlink_topic, self._on_mavlink, mavlink_qos
        )
    ...
    @staticmethod
    def _discover_px4_msg_dir() -> str:
        env_path = os.environ.get("PX4_MSG_DIR", "").strip()
        search_paths = []
        if env_path:
            search_paths.append(Path(env_path))

        repo_relative = Path(__file__).resolve().parents[2] / "messages" / "px4_msgs"
        search_paths.append(repo_relative)
        search_paths.append(Path(__file__).resolve().parents[2] / "messages")
        search_paths.append(Path("/svea_ws/src/messages/px4_msgs"))
        search_paths.append(Path("/svea_ws/src/messages"))
        search_paths.append(Path("/svea_ws/src/px4_msgs"))

        for path in search_paths:
            if path.is_dir() and (any(path.glob("*.msg")) or any(path.rglob("*.msg"))):
                return str(path)
        return ""
    ...
    @staticmethod
    def _payload64_to_bytes(msg: Mavlink) -> bytes:
        out = bytearray()
    ...
    def _extract_tunnel_payload(self, msg: Mavlink) -> Optional[tuple[bytes, int]]:
        if int(msg.msgid) != self.TUNNEL_MSG_ID:
            return None

        data = self._payload64_to_bytes(msg)
        if len(data) < self.TUNNEL_MIN_WIRE_LEN:
            return None

        payload_type = int(
            struct.unpack_from("<H", data, self.TUNNEL_OFFSET_PAYLOAD_TYPE)[0]
        )
        payload_len = int(data[self.TUNNEL_OFFSET_PAYLOAD_LEN])

        if payload_len > 128:
            return None

        payload_start = self.TUNNEL_OFFSET_PAYLOAD
    ...
    def _on_mavlink(self, msg: Mavlink) -> None:
        if int(msg.msgid) != self.TUNNEL_MSG_ID:
            return

        self._tunnel_seen += 1
        extracted = self._extract_tunnel_payload(msg)
        ...
        payload, payload_type = extracted
        ...
        if (self._tunnel_seen % 200) == 0:
            self.get_logger().info(
                ...
                f"decode_errors={self._decode_errors} unknown_topic={self._unknown_topic} "
